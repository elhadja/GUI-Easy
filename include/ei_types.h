...
/**
 * @brief Point_f A 2D point width float coordinates
 */
typedef Vector<float> Point_f;
/**
...
class Rect_f {
public:
    Point_f top_left;  ///< Coordinates of the top-left corner of the rectangle.
    Size size;       ///< Size of the rectangle.
    Rect_f(Point top_left = Point(), Size size = Size())
        : top_left(top_left), size(size) {}
};
...
static font_t antiwarning = default_font;//empéche warnings: "default font is defines but not used"
...
static const int default_button_corner_radius = 10; //10  ///< The default corner radius of button widgets.

/********* default frame values *************** */
static const int default_frame_border_width = 0;
static const color_t default_text_color = {0xff, 0xff, 0xff, 0xff};
}
