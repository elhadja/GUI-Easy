/**
  *\file ei_draw.cpp
  *\brief contains definitions of function in \ref ei_draw.h
...
void ei_set_clipping(const Rect& clipper){
        al_set_clipping_rectangle(clipper.top_left.x(), clipper.top_left.y(), static_cast<int>(clipper.size.width()), static_cast<int>(clipper.size.height()));
}

void ei_draw_filled_rounded_rect(surface_t surface, const Rect *clipper, const Rect& pos_size, color_t color){
    assert(surface != nullptr);
    al_set_target_bitmap(static_cast<ALLEGRO_BITMAP *>(surface));
    if(clipper != nullptr){
      ei_set_clipping(*clipper);
    }
    al_draw_filled_rounded_rectangle(pos_size.top_left.x(), pos_size.top_left.y(), pos_size.top_left.x() + pos_size.size.width(), pos_size.top_left.y() + pos_size.size.height(),
                                     0, 0, al_map_rgba(color.red, color.green, color.blue, color.alpha));
    ei_set_clipping(hw_surface_get_rect(surface));
}
